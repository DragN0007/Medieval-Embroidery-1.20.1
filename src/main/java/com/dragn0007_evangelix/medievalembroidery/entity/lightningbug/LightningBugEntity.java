...
public class LightningBugEntity extends Parrot implements IAnimatable {

    private final AnimationFactory factory = GeckoLibUtil.createFactory(this);

    public LightningBugEntity(EntityType<? extends Parrot> entityType, Level level) {
        super(entityType, level);
        this.noCulling = true;
        this.entityData.define(DARK_TICKS_REMAINING, 0);
    }

    private static final EntityDataAccessor<Integer> DARK_TICKS_REMAINING = SynchedEntityData.defineId(LightningBugEntity.class, EntityDataSerializers.INT);
   ...
   protected float playFlySound(float p_191954_1_) {
    ...
    public ResourceLocation getTextureLocation() {
        return LightningBugModel.Variant.variantFromOrdinal(getVariant()).resourceLocation;
    }

    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(LightningBugEntity.class, EntityDataSerializers.INT);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor levelAccessor, DifficultyInstance difficultyInstance, MobSpawnType mobSpawnType, @Nullable SpawnGroupData spawnGroupData, @Nullable CompoundTag compoundTag) {
        ...
        setVariant(new Random().nextInt(LightningBugModel.Variant.values().length));
