...
public class HealingCrystalBallEntity extends BlockEntity implements GeoBlockEntity {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	public int tickCount;
	private final List<BlockPos> effectBlocks = Lists.newArrayList();

	public HealingCrystalBallEntity(BlockPos pos, BlockState state) {
		super(MEBlocks.HEALING_CRYSTAL_BALL_ENTITY.get(), pos, state);
	}

	public static void serverTick(Level level, BlockPos pos, BlockState state, HealingCrystalBallEntity entity) {
		++entity.tickCount;
		...
		if (!list.isEmpty()) {
			for(Player player : list) {
				if (pos.closerThan(player.blockPosition(), j)) {
					player.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 260, 0, true, true));
				}
			}
